...
class Settings:
    ...
    def miniapp_deeplink(self, start_param: str = "") -> str:
        # Формат диплинка мини-приложения в MAX: https://max.ru/<botName>?startapp[=param]
        # Подробнее: https://dev.max.ru/docs/webapps/introduction
        base = f"https://max.ru/{self.bot_username}?startapp"
        return f"{base}={start_param}" if start_param else base
