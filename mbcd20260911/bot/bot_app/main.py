...
# Делает импорты вида "from bot_app.config import ..." рабочими независимо от того,
# запущен ли файл напрямую (клик в PyCharm) или как модуль (python -m bot_app.main).
# Без этого прямой запуск файла падает с "ModuleNotFoundError: No module named 'bot_app.config'".
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from maxapi import Bot, Dispatcher

from bot_app.config import settings
from bot_app.handlers import menu, start
