# moex_api/database/stock_foreign/writer_stock_foreign_m1_clickhouse.py
...
class StockForeignM1WriterClickHouse:
    """
    Загрузчик минутных свечей зарубежных акций MOEX в ClickHouse.
    ...
    TABLE_NAME = "moex_api.moex_stock_foreign_m1"
    BATCH_SIZE = 5000
    ...
    def __init__(self, clickhouse: StockForeignClickHouse) -> None:
        ...
        logger.debug("StockForeignM1WriterClickHouse initialized TABLE=%s", self.TABLE_NAME)
    ...
    def _prepare_row(
        ...
        """
        logger.debug("PREPARE STOCK FOREIGN M1 ROW SECID=%s DATE=%s", stock.secid, candle.begin)
    ...
    def write(
        ...
        """Запись минутных свечей зарубежных акций чанками в ClickHouse."""
        if not candles:
            logger.warning("NO CANDLES FOR INSERT TABLE=%s", self.TABLE_NAME)
            print("NO DATA FOR INSERT")
            return

        logger.info("CLICKHOUSE STOCK FOREIGN M1 INSERT START TABLE=%s INPUT=%s", self.TABLE_NAME, len(candles))
        print("=" * 80)
        print("CLICKHOUSE INSERT STOCK FOREIGN M1")
        print("=" * 80)
        ...
        for start in range(0, len(rows), self.BATCH_SIZE):
            ...
            try:
                ...
                logger.exception("CLICKHOUSE STOCK FOREIGN M1 INSERT ERROR TABLE=%s ERROR=%s", self.TABLE_NAME, error)
                raise

            inserted += len(batch)

        logger.info("CLICKHOUSE STOCK FOREIGN M1 INSERT COMPLETE TABLE=%s INSERTED=%s", self.TABLE_NAME, inserted)
        print()
