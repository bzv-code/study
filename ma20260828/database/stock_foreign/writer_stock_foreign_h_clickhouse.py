# moex_api/database/stock_foreign/writer_stock_foreign_h_clickhouse.py
...
class StockForeignHWriterClickHouse:
    """
    Загрузчик недельных свечей зарубежных акций MOEX в ClickHouse.

    Таблица:
        moex_api.moex_stock_foreign_h

    Проверка дублей:
        secid + date
    """

    TABLE_NAME = "moex_api.moex_stock_foreign_h"
    BATCH_SIZE = 5000
    ...
    def __init__(self, clickhouse: StockForeignClickHouse) -> None:
        ...
        logger.debug("StockForeignHWriterClickHouse initialized TABLE=%s", self.TABLE_NAME)
    ...
    def _prepare_row(
        ...
        """
        logger.debug("PREPARE STOCK FOREIGN H ROW SECID=%s DATE=%s", stock.secid, candle.begin)
    ...
    def write(
        ...
        """Запись недельных свечей зарубежных акций чанками в ClickHouse."""
        if not candles:
            logger.warning("NO CANDLES FOR INSERT TABLE=%s", self.TABLE_NAME)
            print("NO DATA FOR INSERT")
            return

        logger.info("CLICKHOUSE STOCK FOREIGN H INSERT START TABLE=%s INPUT=%s", self.TABLE_NAME, len(candles))
        print("=" * 80)
        print("CLICKHOUSE INSERT STOCK FOREIGN H")
        print("=" * 80)
        ...
        for start in range(0, len(rows), self.BATCH_SIZE):
            ...
            try:
                ...
                logger.exception("CLICKHOUSE STOCK FOREIGN H INSERT ERROR TABLE=%s ERROR=%s", self.TABLE_NAME, error)
                raise

            inserted += len(batch)

        logger.info("CLICKHOUSE STOCK FOREIGN H INSERT COMPLETE TABLE=%s INSERTED=%s", self.TABLE_NAME, inserted)
        print()
