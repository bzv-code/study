# moex_api/database/stock_foreign/writer_stock_foreign_d_clickhouse.py
...
class StockForeignDWriterClickHouse:
    """
    Загрузчик дневных свечей зарубежных акций MOEX в ClickHouse.
    ...
    TABLE_NAME = "moex_api.moex_stock_foreign_d"
    BATCH_SIZE = 5000
    ...
    def __init__(self, clickhouse: StockForeignClickHouse) -> None:
        ...
        logger.debug("StockForeignDWriterClickHouse initialized TABLE=%s", self.TABLE_NAME)
    ...
    def _prepare_row(
        ...
        """
        logger.debug("PREPARE STOCK FOREIGN ROW SECID=%s DATE=%s", stock.secid, candle.begin)
    ...
    def write(
        ...
        """Запись дневных свечей зарубежных акций чанками в ClickHouse."""
        if not candles:
            logger.warning("NO CANDLES FOR INSERT TABLE=%s", self.TABLE_NAME)
            print("NO DATA FOR INSERT")
            return

        logger.info("CLICKHOUSE STOCK FOREIGN INSERT START TABLE=%s INPUT=%s", self.TABLE_NAME, len(candles))
        print("=" * 80)
        print("CLICKHOUSE INSERT STOCK FOREIGN D")
        print("=" * 80)
        ...
        logger.info("CLICKHOUSE STOCK FOREIGN INSERT COMPLETE TABLE=%s INSERTED=%s", self.TABLE_NAME, inserted)
        print()
