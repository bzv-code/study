# moex_api/database/stock_foreign/writer_stock_foreign_m10_clickhouse.py
...
class StockForeignM10WriterClickHouse:
    """
    Загрузчик 10-минутных свечей зарубежных акций MOEX в ClickHouse.

    Таблица:
        moex_api.moex_stock_foreign_m10
    ...
    def __init__(self, clickhouse: StockForeignClickHouse) -> None:
        ...
        logger.debug("StockForeignM10WriterClickHouse initialized TABLE=%s", self.TABLE_NAME)
    ...
    def _prepare_row(
        ...
        """
        logger.debug("PREPARE STOCK FOREIGN M10 ROW SECID=%s DATE=%s", stock.secid, candle.begin)
    ...
    def write(
        ...
        """Запись 10-минутных свечей зарубежных акций чанками в ClickHouse."""
        if not candles:
            logger.warning("NO CANDLES FOR INSERT TABLE=%s", self.TABLE_NAME)
            print("NO DATA FOR INSERT")
            return

        logger.info("CLICKHOUSE STOCK FOREIGN M10 INSERT START TABLE=%s INPUT=%s", self.TABLE_NAME, len(candles))
        print("=" * 80)
        print("CLICKHOUSE INSERT STOCK FOREIGN M10")
        print("=" * 80)
        ...
        for start in range(0, len(rows), self.BATCH_SIZE):
            ...
            try:
                ...
                logger.exception("CLICKHOUSE STOCK FOREIGN M10 INSERT ERROR TABLE=%s ERROR=%s", self.TABLE_NAME, error)
                raise

            inserted += len(batch)

        logger.info("CLICKHOUSE STOCK FOREIGN M10 INSERT COMPLETE TABLE=%s INSERTED=%s", self.TABLE_NAME, inserted)
        print()
